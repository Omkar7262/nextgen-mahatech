...
export default function Contact() {
  ...
  const [content, setContent] = useState<any>({
    address: "Shree Ganesh Park, Near Patil Park, Nashik – 422010, Maharashtra, India",
    phone: "+91 95794 95373",
    email: "nextgenmahatech@gmail.com",
  ...
  useEffect(() => {
    fetch("/api/content/contact")
      .then(res => res.json())
      .then(data => {
        if (data.success && data.data.info) {
          setContent({
            address: data.data.info.address || content.address,
            phone: data.data.info.phone || content.phone,
            email: data.data.info.email || content.email,
            hours: data.data.info.hours || content.hours,
          });
        }
      });
  }, []);
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <Reveal3D className="h-full">
            ...
            <div className="mt-10 space-y-5">
              <ContactRow icon={phoneIcon} label="Phone" value={content.phone} href={`tel:${content.phone.replace(/\s+/g, '')}`} />
              <ContactRow icon={mailIcon} label="Email" value={content.email} href={`mailto:${content.email}`} />
              <ContactRow
                icon={pinIcon}
                label="Office"
                value={content.address}
              />
              <ContactRow icon={clockIcon} label="Working Hours" value={content.hours} />
            </div>
